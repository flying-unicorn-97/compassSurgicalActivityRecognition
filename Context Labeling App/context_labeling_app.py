...
threadStates = ["Loose", "Taut"]         # 松的，拉紧的
...
class App:
    def __init__(self, window, window_title, video_source=0, labelTarget=None):
        ...
        streaming = True
        self.window = window
        self.window.title(window_title)    # task_subject_trial
        ...
        self.frame_stride = int(self.vid.fps * (1.0/sps))   # 每步帧数序号的增量
    ...
    def updatePreview(self):
        ...
        self.prev.after(100, self.updatePreview)      # 每隔100ms调用updatePreview函数
    ...
    def mark(self, etc=False):
        ...
        if( str(frameNum) in record):
            print(record[str(frameNum)])
            self.LHoldOpts.config(bg="#85F280")        # 绿色
            self.LContactOpts.config(bg="#85F280")
            self.RHoldOpts.config(bg="#85F280")
            self.RContactOpts.config(bg="#85F280")
        else:
            self.LHoldOpts.config(bg="#F0F0F0")         # 灰色
            self.LContactOpts.config(bg="#F0F0F0")
            self.RHoldOpts.config(bg="#F0F0F0")
            self.RContactOpts.config(bg="#F0F0F0")
    ...
    def saveToFile(self):
        ...
        o = open(self.labelTarget, 'w+')            # 文件已存在内容清空，重新写入
        row = 0
        #print(record)
        ...
        while(str(row) in record):
            ...
            o.write(str(objects.index(data[2])) + " " + str(objects.index(data[3])) + " ")  # right grasper
...
class MyVideoCapture:
    ...
    def get_frame(self):
        if self.vid.isOpened():
            self.vid.set(1, frameNum)   # 1，从0开始的解码/捕获时间帧，frameNum帧的序号
    ...
    def get_frame_prev(self, previewFrame):
        if self.vid.isOpened():
            ...
            if ret:
                ...
                frame=cv2.resize(frame, (newwidth, 480), interpolation=cv2.INTER_AREA)
                '''
                # Return a boolean success flag and the current frame of format BGR converted to RGB
                return (ret, cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))
            else:
                return (ret, None)
        else:
...
dir=os.path.dirname(os.getcwd())   # the parent directory of cwd
...
# doneList = [done.split('\\')[-1].rsplit(".txt")[0] for done in glob.glob(transcriptDir+'\\*.txt')]  #for windows
...
# videos = glob.glob(videoDir+"\\*.avi") + glob.glob(videoDir+"\\*.mp4")    #for windows
