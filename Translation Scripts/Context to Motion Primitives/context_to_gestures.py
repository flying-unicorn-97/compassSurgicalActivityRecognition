...
def condenseContext(df):
    ...
    dfContext['Frame'] = df.iloc[:, 0]    # ‘Frame' 帧序号
    #print(dfContext)


    # concatenate states into one string
    # initialize with first context variable
    dfContext['Context'] = df.iloc[:,1]  # 'Context' 五个数字构成的动作文本标签，字符串
    for i in range(2, numContext+1):
        #print(i)
        dfContext['Context'] = dfContext['Context'].map(str) + df.iloc[:,i].map(str)  # 将列数字文本转化为字符串并连接，如"00000"，“12234”
...
def group(dfContext):
    ...
    dfGrouped = dfContext.groupby('subgroup', as_index=False).apply(lambda x: pd.DataFrame([(x["Frame"].iloc[0],
                                                                                             x["Frame"].iloc[-1],
                                                                                             x["Context"].iloc[0])],
                                                                                           columns=["Start", "End",
                                                                                                    "Context"]))
    myDF = dfGrouped.reset_index(drop=True)
    #head(1)))  x.head(1)
    #as_index=False,保留分组之前的索引，'subgroup'不替换之前的索引
    #print(dfGrouped)
...
def labelGs(npGrouped):
    ...
    # gestures included in task S, NP or KT(不一定对)
    # S:G1, 2, 3, 4, 5, 6, 8, 9, 10, 11
    # NP:G1, 2, 3, 4, 5, 6, 8, 11
    # KT:G1 , 11 , 12 , 13 , 14 , 15
        ...
        if (task == "Suturing") or (task == "Needle_Passing"):
            ...
            if (entry == 1):
                ...
                if ((currState == "20200") and (nextState == "00200")) or ((currState == "02200") and (nextState == "00200")):
                    ...
                    Gs[1] = str(currStop)
                    currStart = currStop+1
                    Gs.append("G8(LR, Needle)") #Gs.append("G8(LR, Needle)")
                    #print("G8 found")
                    #print(Gs)
...
gDir = os.path.join(taskDir, "gestures_test")     #将transcriptions_context下的动态描述文件转化为姿势


# Based on task, create the context state list
if (task == "Suturing") or (task == "Needle_Passing"):
    context = GContext + SNPContext                    # 列表合并
