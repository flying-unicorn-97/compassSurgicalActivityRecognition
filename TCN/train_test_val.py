...
def train_model(config,type,train_dataset,val_dataset,input_size, num_class,num_epochs,
    ...
    if loss_weights is None:
        ...
        criterion = nn.CrossEntropyLoss(
                        ...
                        ignore_index=-1)     # 数据处理标签扩充元素填充为-1,ignore_index=-1表示标签为-1的预测值和真实值不参与交叉熵损失计算
    ...
    for epoch in range(num_epochs):
        ...
        running_loss = 0.0
        epoch_steps = 0
        for i, data in enumerate(train_loader):   #一个batch数据用于模型训练
            ...
            if i % 10 == 9:  # print every 2000 mini-batches
                print("[epoch %d, step %5d] loss: %.3f" % (epoch + 1, i + 1,
...
def train_model_parameter( config, type,input_size, num_class,num_epochs,dataset_name,sample_rate,
    ...
    if val_type == "LOSO":
        paths = get_cross_val_splits(validation = True)    # trial is digits
    elif val_type == "LOUO":                               # trial is str
    ...
    if  dataset_name=="All-5a":
        ...
        train_trail_list.extend(other_train)        # extend 去掉变量的list最外层符号
    ...
    if checkpoint_dir:
        model_state, optimizer_state = torch.load(
            os.path.join(checkpoint_dir, "checkpoint"))
        model.load_state_dict(model_state)             # 加载不带模型结构的模型参数
        optimizer.load_state_dict(optimizer_state)
    ...
    for epoch in range(num_epochs):
        ...
        for i, data in enumerate(train_loader):
            feature = data['feature'].float()
            feature = feature.to(device) #.cuda()       # shape:(1,nsamples,nfeatures)

            gesture = data['gesture'].long()
            gesture = gesture.view(-1)
            gesture = gesture.to(device) #.cuda()       # shape:(nsamples)

            # Forward
            out = model(feature)
            """
            if epoch == 0 and i == 0:
                print("[epoch 0,model out shape: {}".format(out.size()))   # (1,nsamples,num_class)
            """
            flatten_out = out.view(-1, out.shape[-1])
            """
            if epoch == 0 and i == 0:
                print("[epoch 0, model flatten_out shape: {}".format(flatten_out.size()))  #(nsamples,num_class)
            """
            #breakpoint()
...
def test_model(model, test_dataset, loss_weights=None, log_dir =None, name = 'default',plot_naming=None, epoch='default'):
    ...
    with torch.no_grad():      # 模型参数不进行梯度计算
        for i, data in enumerate(test_loader):
            # Access name of kinematic file and pull out the trial name (task, subject, and trial number)
            trialName = data['name'][0].split("/")[-1].split(".")[0]   # 获取文件名：task_subject_trial
            naming=data['name'][0].split("/")[-1][:-4]                 # 同trialName(去除文件名后缀)
            #print(trialName)
            ...
            # Forward
            out = model(feature)               # (1,nsamples,num_class)
            """
            # for debug
            if(i==0):
            ...
            """
            out = out.squeeze(0)                  # nsamples*num_class

            loss = criterion(input=out, target=gesture)

            total_loss += loss.item()

            pred = out.data.max(1)[1]       #[0] 返回最大值；[1]返回最大值索引

            trail_len = (gesture.data.cpu().numpy()!=-1).sum()
            gesture = gesture[:trail_len]
            pred = pred[:trail_len]

            preditions.append(pred.cpu().numpy())   # 数字标签
            gts.append(gesture.data.cpu().numpy())
            if log_dir!=None and name !='train':
                model_conv_pred = label_transform.inverse_transform(pred.cpu().numpy())  # 数据标签转化为文本标签
            #breakpoint()
                ...
                #me
                #np.save(os.path.join(log_dir, 'epoch_{}'.format(epoch), test_data_naming_pred_gt),\
                #        [data['feature'][:, :trail_len].float(), model_conv_pred, model_conv_gt])
    ...
    # 当样本不均衡（某些标签的样本数远远超过其余标签样本数），不能单独用正确率来评判模型的优劣
...
def cross_validate(dataset_name,net_name, logDir):
    ...
    elif (valtype == "LOUO") and (dataset_name == "PTPaS"):    # 两种任务混合在一块
    ...
    # Cross Validation
    for idx, data in enumerate(cross_val_splits):     #每一折的训练和测试集
        #breakpoint()
        # Dataset
        ...
        import fnmatch
        if  dataset_name == "All-5a":
            test_dir_5a=[dir  for dir in test_dir if fnmatch.fnmatch(dir,"*[Suturing,Needle_Passing,Knot_Tying]*")]   #fnmatch文件名匹配,fnmatch.fnmatch返回True or False
        ...
        os.makedirs(trained_model_dir, exist_ok=True)  # os.makedirs创建多层目录，exist_ok=True不触发目录存在的异常
...
if __name__ == "__main__":
    ...
    now = datetime.now()
    timeNow = now.strftime("%Y_%m_%d_%H_%M_%S")
    logFolder = set +"_"+ var +"_"+ labeltype +"_"+ valtype +"_"+ "trainTestVal_"+timeNow
    ...
    shutil.copy2(configPath, logDir)   # shutil.copy2(src, dst)
    ...
    print("train_test_val.py terminated successfully!")
