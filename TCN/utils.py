...
import numpy as np
import matplotlib
matplotlib.use('AGG')    # Show Plot Disabled  远程服务器上需要
...
def generate_random_str(size, chs=string.ascii_uppercase + string.digits):   #string.ascii_uppercase获取ASCII码中所有的大写字母,string.digit获取所有十进制的数字字符
...
def get_cross_val_splits_random(k=5):
    ...
    def partition(list, n):
        random.shuffle(list)
        return [list[i::n] for i in range(n)]     #每一折的文件列表
    ...
    for i in raw_feature_dir:
        all_files.extend(glob.glob(os.path.join(i, "*")))     #glob.glob返回一维列表,extend只是将列表中的元素的添加到新列表中

    test_folds = partition(all_files, k)           #2×fold_size(fold_size一折含有的文件数)
...
def get_cross_val_splits_LOTO(validation = False):
    # load from config
    print("calling get_cross_val_splits_LOTO,validation: ",validation)
    from config import raw_feature_dir, validation_trial, validation_trial_train, test_trial, train_trial
    ...
    if validation ==True:
        ...
        if len(raw_feature_dir)!=1:
            ...
            print(os.path.join(i, '*_T'+validata_trail[0]+'*'))
            test = glob.glob(os.path.join(i, '*_T' + validation_trial[0] + '*'))
            test_dir.extend(test)
    ...
    # for cross_validate
        ...
        for idx, test_task in enumerate(test_trial):
            train_dir = []
            test_dir = []
            if len(raw_feature_dir)==1:
                print("val type LOTO required less than 2 tasks")
                sys.exit()
            else:
                test = glob.glob(os.path.join(raw_feature_dir[test_task in raw_feature_dir], '*'));
                test_dir = test
                for t in range(len(train_trial)):
                    train = glob.glob(os.path.join(raw_feature_dir[train_trial[t][0] in raw_feature_dir],'*'));
                    train_dir.extend(train)
            #breakpoint()
            # add fold sets to cross_val_splits
            cross_val_splits.append({'train': train_dir,
                                    'test': test_dir,
                                    'name': 'test_{}'.format(test_task)})
...
def get_cross_val_splits_LOUO_all(validation = False):
    ...
    if validation ==True:
        ...
        if len(raw_feature_dir)!=1:
            for i in raw_feature_dir:
                ...
                #me
                print(os.path.join(i, '*S'+validation_trial[0]+'_*'))
                test = glob.glob(os.path.join(i, '*S' +validation_trial[0] + '_*'))
                test_dir.extend(test)
    ...
    # for training
        ...
        data_dir = os.path.dirname(os.path.dirname(i))      #去掉文件名，返回目录
        ...
        for idx, test_num in enumerate(test_trial):
            ...
            if len(raw_feature_dir)!=1:
                for i in raw_feature_dir:
                    ...
                    test = glob.glob(os.path.join(i,'*S'+test_num+'_*'))
                    test_dir.extend(test)
            ...
            cross_val_splits.append({'train': train_dir,
                                    'test': test_dir,
                                    'name': 'test_{}User'.format(test_num)})
...
def get_cross_val_splits_LOUO(validation = False):
    from config import raw_feature_dir, validation_trial, validation_trial_train, test_trial, train_trial
    print("get_cross_val_splits_LOUO,validation: ",validation)
    if validation ==True:
        test_dir = []
        train_dir = []
        if len(raw_feature_dir)!=1:
            for i in raw_feature_dir:
                ...
                #me
                print(os.path.join(i, '*_S'+validation_trial+'_*'))
                test = glob.glob(os.path.join(i, '*_S'+validation_trial+'_*'))
                test_dir.extend(test)                                #一维
                train = glob.glob(os.path.join(i, "*"))
                train = [t for t in train if t not in test]          #一维
                train_dir.extend(train)
        ...
        for idx, test_num in enumerate(test_trial):
            ...
            if len(raw_feature_dir)!=1:
                for i in raw_feature_dir:
                    print(os.path.join(i, '*_S' + test_num + '_*'))
                    test = glob.glob(os.path.join(i,'*_S'+test_num+'_*'))
                    test_dir.extend(test)
                    train = glob.glob(os.path.join(i, "*"))
                    train = [t for t in train if t not in test]
                    train_dir.extend(train)
            ...
            cross_val_splits.append({'train': train_dir,
                                    'test': test_dir,
                                    'name': 'test_User{}'.format(test_num)})
...
def get_cross_val_splits(validation = False):
    from config import raw_feature_dir, validation_trial, validation_trial_train, test_trial, train_trial
    if validation ==True:      # for tuning
        test_dir = []
        train_dir = []
        if len(raw_feature_dir)!=1:  # multi_task
            """
            for i in raw_feature_dir:
            ...
            """
            for i in raw_feature_dir:
                print(os.path.join(i, '*_T{:0>2}.*'.format(validation_trial)))    # 数据宽度为2, 左补0
                test = glob.glob(os.path.join(i, '*_T{:0>2}.*'.format(validation_trial)))
                test_dir.extend(test)
                all_trails = glob.glob(os.path.join(i, '*'))
                train = [dir for dir in all_trails if dir not in test_dir]
                train_dir.extend(train)
            return {'train':train_dir,'test':test_dir,'name':'tune'}

        else:
            i = raw_feature_dir[0]
            print(os.path.join(i,'*_T{:0>2}.*'.format(validation_trial)))                    # 文件路径名匹配
            test = glob.glob(os.path.join(i,'*_T{:0>2}.*'.format(validation_trial)))
            test_dir.extend(test)
            all_trails = glob.glob(os.path.join(i, '*'))
            train = [dir for dir in all_trails if dir not in test_dir]
        ...
        cross_val_splits = []
        for idx, test_num in enumerate(test_trial):    #idx represent the ith fold
            train_dir = []
            test_dir = []
            if len(raw_feature_dir)!=1:
                '''
                ...
                '''
                for i in raw_feature_dir:
                    test = glob.glob(os.path.join(i,'*_T{:0>2}.*'.format(test_num)))
                    test_dir.extend(test)
                    all_trail_dir = glob.glob(os.path.join(i,'*'))
                    train = [dir for dir in all_trail_dir if dir not in test]
                    train_dir.extend(train)
            else:
                i = raw_feature_dir[0]
                test = glob.glob(os.path.join(i,'*_T{:0>2}.*'.format(test_num)))
                ...
                #breakpoint()
                all_trail_dir = glob.glob(os.path.join(i,'*'))
                train = [dir for dir in all_trail_dir if dir not in test_dir]
                train_dir.extend(train)
            #breakpoint()
            cross_val_splits.append({'train': train_dir,
                                    'test': test_dir,
...
def get_class_weights(dataset):  # RAW
    ...
    counts = get_class_counts(dataset)       #每个文件中标签i对应的样本个数
    ...
    w_sum = sum(weights)
    for i in range(class_num):
        weights[i] = weights[i] * class_num / w_sum  #标签i的占比
...
def levenstein_(p,y, norm=False):       # 莱文斯坦距离，字符串A变为字符串B所需要的最少编辑次数
    m_row = len(p)
    n_col = len(y)
...
def segment_labels(Yi):
    idxs = [0] + (np.nonzero(np.diff(Yi))[0]+1).tolist() + [len(Yi)]  # np.zero非零元素的位置，np.diff一阶差分
...
def get_overlap_f1_colin(P, Y, n_classes=0, bg_class=None, overlap=.1, **kwargs):
    def overlap_(p,y, n_classes, bg_class, overlap):
        ...
        precision = TP / (TP+FP)      # 精确率，预测为正类别中真正为正类别的比例
        recall = TP / (TP+FN)         # 召回率，真正为正类别中预测为真类别的比例
